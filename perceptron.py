...
def load_images_from_directory(label, path, x_train_list, y_train_list):
    for img_name in listdir(path):
        x_train_list.append(np.array(Image.open(f'{path}{img_name}').convert('L').resize(target_size)) / 255.0)
...
def create_model():
    ...
    m.add(keras.layers.Dense(128, activation=keras.activations.relu))
    m.add(keras.layers.Dropout(0.1))
    m.add(keras.layers.Dense(128, activation=keras.activations.relu))
    m.add(keras.layers.Dropout(0.1))
    m.add(keras.layers.Dense(128, activation=keras.activations.relu))
    m.add(keras.layers.Dropout(0.1))
    ...
    #m.compile(optimizer=keras.optimizers.SGD(learning_rate=0.01, name="SGD"),
    #m.compile(optimizer=keras.optimizers.Adagrad(learning_rate=0.01, name="Adagrad"),
    m.compile(optimizer=keras.optimizers.Adamax(learning_rate=0.001, name="Adamax"),
...
if __name__ == "__main__":
    ...
    #20 epochs for Adam
    #20 epochs for Adamax
    #50 epochs for SGD
    #50 epochs for adagrad

    logs = model.fit(x_train, y_train, epochs=40,
                     batch_size=4,
